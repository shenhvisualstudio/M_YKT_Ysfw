...
namespace M_YKT_Ysfw.Service.Problem
{
    public partial class SubmitProblem : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            log.strBehavior = "提交问题";
            log.strBehaviorURL = "/Service/Problem/SubmitProblem.aspx";
            ...
            try
            {
                ...
                {
                    Json = JsonConvert.SerializeObject(new DicPackage(false, null, "参数错误，提交问题失败").DicInfo());
                    return;
                }
            ...
            {
                Json = JsonConvert.SerializeObject(new DicPackage(false, null, string.Format("{0}：提交问题发生异常。{1}", ex.Source, ex.Message)).DicInfo());
                log.LogCatalogFailure(string.Format("{0}：提交问题发生异常。{1}", ex.Source, ex.Message));
            }
        }
