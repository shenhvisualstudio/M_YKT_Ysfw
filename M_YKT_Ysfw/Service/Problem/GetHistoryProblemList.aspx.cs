//
//文件名：    GetHistoryProblemList.aspx.cs
//功能描述：  获取历史问题列表
//创建时间：  2017/10/08
//作者：      
//修改时间：  
...
{
    public partial class GetHistoryProblemList : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Json = JsonConvert.SerializeObject(new DicPackage(false, null, "参数错误，获取历史问题列表数据失败").DicInfo());
                    return;
                }
                ...
                string strSql =
                    string.Format(@"select a.id,a.type_mark,c.complain_name,b.department,a.vehicle,a.driverphone,to_char(a.createtime, 'yyyy/mm/dd') as createtime,a.anonymous_mark,a.finish_mark,a.title,a.detail,a.adjust_time,a.deal_time,a.evaluate_time,e.complain_name as complain_name_org,d.department as department_org
                                    ...
                                    where a.type_mark='1' and a.account='{0}' and a.finish_mark='1'
            ...
            {
                Json = JsonConvert.SerializeObject(new DicPackage(false, null, string.Format("{0}：获取历史问题列表数据发生异常。{1}", ex.Source, ex.Message)).DicInfo());
            }
        }
